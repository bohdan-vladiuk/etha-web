import { ContactUsForm, SubscribeToNewsLetter } from '../models/ContactUs';
import { setLoaderVisibility } from '../redux';
import { AppDispatch } from '../redux/store';
import { CONTACT_US_SUBMIT, JOIN_WAITLIST } from '../services/API';
import api from '../services/api-helper';

export async function ContactUs(contactUsForm: ContactUsForm, cleanFunction: () => void): Promise<void> {
    api.post(CONTACT_US_SUBMIT, contactUsForm).then(
        (response) => {
            if (response.data !== undefined && response.data.id !== '') {
                cleanFunction();
            } else {
                console.log();
            }
        },
        (err) => {
            alert('Please check the entered values');
            console.log('Error: ', err);
        },
    );
}

export async function Subscribe(submitSubscribe: SubscribeToNewsLetter, cleanFunction: () => void): Promise<void> {
    api.post(JOIN_WAITLIST, submitSubscribe).then(
        (response) => {
            if (response.data !== undefined && response.data.id !== '') {
                cleanFunction();
            } else {
                console.log();
            }
        },
        (err) => {
            alert('Please check the entered values');
            console.log('Error: ', err);
        },
    );
}
